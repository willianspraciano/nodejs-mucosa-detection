...
function newArray(color, outputName){
  let arr = {};
  arr.red = Math.round(Number(color[0]));
  arr.green = Math.round(Number(color[1]));
  arr.blue = Math.round(Number(color[2]));
  arr.file = outputName;
  arr.label = 'T';
...
function getColor(outputName) {
  const imageCropped = path.resolve(__dirname, 'output', outputName);
  
  color(imageCropped, { format: 'rgb' }, function (err, color ) {
...
async function getMucosa(imageFolder, imageName) {
  ...
  const imagePath = path
    .resolve(__dirname, imageFolder, imageName);

  const image = await fs.readFile(imagePath);
  const tensor = tf.node.decodeImage(image);

  const graphModel = await tf.loadGraphModel(modelJSON);
  const model = new automl
    .ObjectDetectionModel(graphModel, ['background','Mucosa']);
